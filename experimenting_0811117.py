...
# -----------------------------------------------------------------#
muVals = scalesArray(1, 10**6, 20)  # (GeV) mass of DM #np.array([T0]) #
print muVals
...
for mu in muVals:
	...
	#-----------------------------#
		...
		# heavy SM vals
		mbh_H = 1 / (8 * pi * G * SM_heavy.mass)			# BH mass when SM_h produced
		t_smH = tmu + (mbh_H ** 3 - mbh_mu ** 3) / (3. * AsmLDM)	# time when SM_h produced
		if t_smH < t0:
			t_smH = t0
			mbh_H = mbh0
		...
		def m(t): # gives mass at any time, accounting for all epochs
			if t <= tmu:
				...
				result = m_(t, tmu, AsmLDM, mbh_mu)
			else:
				result = m_(t, t_smH, Atot, mbh_H)
			return result

		t_end = t_smH + ((10*Mpl)**3 - mbh_H ** 3) / (3. * Atot)
